...
export default function SendAlert({ onSend }: SendAlertProps) {
    ...
    const [zone, setZone] = useState('My Area');

    const submit = () => {
        if (!title.trim()) return;
        onSend({ title, description: desc, severity, zone, source: 'Manual Report' });
        setTitle('');
        setDesc('');
        setOpen(false);
    };
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-medium text-[var(--text)]">New Alert</h2>
                <button
